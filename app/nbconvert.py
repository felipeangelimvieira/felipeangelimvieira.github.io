...
class NbConvertPlugin(BasePlugin[_PluginConfig]):
    def on_files(self, files, config, **kwargs):
        ...
        output_dir = path.realpath(
            path.join(config["docs_dir"], path.normpath(self.config["output_dir"]))
        )
        ...
        for i, nb_path in enumerate(nb_finder, 1):
            ...
            if exe_opts is not None:
                ...
                try:
                    ...
                    exe_completed = True
                    log.error(
                        "[NbConvertPlugin] (%d) notebook execution error(%.3fs): %s",
                        i,
                        time() - ts,
                        err,
                    )
                    ...
                    exe_completed = True
                    log.debug(
                        "[NbConvertPlugin] (%d) notebook execution finish(%.3fs)",
                        i,
                        time() - ts,
                    )
                finally:
    ...
    def on_post_build(self, config, **kwargs):
        ...
        output_dir = path.join(
            config["docs_dir"], path.normpath(self.config["output_dir"])
        )
        ...
        try:
            ...
            log.warning(
                "[NbConvertPlugin] OSError on removedirs %r: %s", output_dir, err
            )
