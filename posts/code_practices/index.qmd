...
Bringing it to the context of this post, we can say that **product requirements are not the product we are building**. It is a simplification of what
we want to build. Naturally, details can be missing and assumptions can be wrong, because sometimes
...
Basically, simplicity implies less effort on the long term.

Think about the code as a graph. The nodes are components (it could be a function for example) we create, and the edges represent their dependencies. Simple code would be a graph with few edges, where edges represent dependencies of components on others. A dependency can be, for example, an assumption of the structure of the output of one component, the existence of certain methods, etc. The number of edges would be proportional to the complexity of the system and to our cognitive load. 
